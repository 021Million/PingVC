...
- July 16, 2025. Enhanced Airtable integration to capture ALL project setup submissions including drafts. Added new "Project Submissions" table that tracks every form submission from /project-setup page regardless of publishing status. This provides comprehensive lead generation and funnel analytics for all founder activity, not just published projects. Created detailed table setup guide with field specifications and configuration options.
- July 16, 2025. Implemented comprehensive project verification system. Added verification status fields to founders schema (under_review, verified, rejected) with admin controls. Projects start as "Under Review" and display clear status badges in Scout marketplace thumbnails. Added admin API endpoints for managing verification status and notes. All projects remain on same marketplace with visual status indicators.
- July 16, 2025. Added authentication protection to project submission workflow. Users must sign up or log in before accessing /project-setup page. Unauthenticated users are redirected to /auth page. Updated Scout marketplace CTA to show "Sign Up to Publish" for unauthenticated users and "Publish Your Project" for logged-in users. All project submission flows now require authentication first.
- July 16, 2025. Implemented user type access control restrictions. VCs and Angels are now blocked from accessing project submission features (/project-setup) and are redirected to investor browse page (/vcs) with appropriate messaging. Updated Scout page CTAs to show "Browse Investors" for VCs/Angels and "Publish Your Project" for founders. Fixed display of "VC" instead of "vc" in profile badges and account type fields. Project submission functionality is now exclusively available to founders only.
- July 16, 2025. Enhanced investor profile activity history to display comprehensive tracking for VCs and Angels. Added "Requests Received" section showing founder connection requests with project details, founder information, and request status. Added "Calls Booked" section displaying paid bookings with founder details, project names, payment amounts, and booking status. Created dedicated API endpoint /api/profile/investor-activity and updated profile page to show differentiated activity views for founders (outbound requests/unlocks) vs investors (inbound requests/bookings).
- July 16, 2025. Implemented comprehensive "List Project Now" button system across all major pages. Created reusable ListProjectButton component with three variants (default, floating, large) that automatically handles authentication flow - redirects unauthenticated users to auth page and authenticated founders to project-setup page. Added prominent green "List Project Now" buttons to landing page, home page, Scout marketplace, and VCs page. VCs and Angels don't see these buttons to maintain role-appropriate CTAs. Includes floating action button on Scout and VCs pages for easy access to project submission.
- July 16, 2025. Enhanced Scout marketplace with prominent "List Project Now" button in header section. Added direct button implementation that bypasses role-based hiding to ensure visibility for all users. Changed page title from "Scout Marketplace" to simply "Marketplace" for cleaner branding. Button includes authentication flow and green styling for maximum visibility.
- July 16, 2025. Updated marketplace data source to pull projects from "Project Submissions" Airtable table instead of "Scout" table. Marketplace now displays all projects with valid company names from project submissions, providing unified data flow from project setup form directly to marketplace visibility. Fixed field mapping to match Project Submissions table structure and ensured proper spacing and data normalization for VC profiles with array field handling.
- July 16, 2025. Implemented comprehensive role-based signup flow with proper routing. Updated "Sign up as VC" buttons to direct to `/signup` page with user type selection (VC, Angel, or Founder). Added URL parameter support in auth system for default user type selection. Created smart redirect logic after registration: founders go to `/project-setup`, VCs/Angels go to `/vc-signup`. Enhanced Top VCs leaderboard with clickable entries that properly route to correct profile pages and added scroll-to-top functionality for better UX.
- July 16, 2025. Implemented investor type categorization system allowing VCs to specify their type as "Venture Capital" and/or "Angel" investor through checkboxes in VC signup form. Added is_venture_capital and is_angel boolean fields to database schema, integrated form validation, and enhanced profile display with investor type badges. Updated signup flow to properly route both VCs and Angels through auth page first, then to VC signup form after account creation. Updated profile page to show "Signed up" instead of "Member since" for clearer user experience.